...
int checkDate3(int d, int m, int y){	// Same implementation as in first problem
	if (m>0 && m<13){
		if (m == 2){
...
int compareDates3(char *dob1, char *dob2) {
	...
	if (checkDate3(d1, m1, y1) == 0)
		return -1;
	if (checkDate3(d2, m2, y2) == 0)
		return -2;
	else{
		if (y1>y2)
			return 2;
...
struct transaction * mergeSortedArrays(struct transaction *A, int ALen, struct transaction *B, int BLen) {
	...
	while (AInd < ALen && BInd < BLen) {
		int temp = compareDates3(A[AInd].date, B[BInd].date);
